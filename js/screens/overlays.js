...
import { settings, loadQuiz, saveQuiz, logEvent, addXp, addPrize,
         ...
         quizQuestionKey, QXP_ATTEMPT, QXP_CORRECT } from "../store.js";
...
export function finishQuizDeck(qd) {
  ...
  if (paid) {
    qd.qs.forEach((q, ix) => {
      ...
      const key = quizQuestionKey(q.move, q.kind);
      const rec = quiz.qLedger[key] || { attempted: false, mastered: false };
      if (!rec.attempted) { rec.attempted = true; xp += QXP_ATTEMPT; firstSeen++; }
      if (!rec.mastered && q.opts[pick] && q.opts[pick].ok) {
        rec.mastered = true; xp += QXP_CORRECT; newlyMastered++;
      }
      quiz.qLedger[key] = rec;
    });
