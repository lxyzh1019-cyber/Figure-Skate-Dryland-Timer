...
function settingsTab(vm) {
  ...
  <div style="background:var(--surface);border:1.5px solid var(--hairline);border-radius:var(--radius-xl);padding:22px;box-shadow:var(--shadow-soft);display:flex;flex-direction:column;gap:22px;max-width:520px;">
    ...
    <div style="border-top:1.5px solid var(--hairline);padding-top:16px;">
      ...
      <div style="font-size:13px;font-weight:700;color:var(--ink-soft);margin-bottom:9px;line-height:1.5;">A full copy of <strong>${escapeHtml(vm.settingsName)}</strong>'s data — sessions, XP, prizes, quiz mastery, trackers. Restoring only adds; nothing already on this device is overwritten. Keep a copy somewhere safe: a phone that clears its browser data has no other way back.</div>
