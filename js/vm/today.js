/* ============================================================
   ...
   Today slice, fed by real data: DAYS content, skate_sessions_v2
...
export function planStats(day) {
  ...
  let moves = 0, secs = 0;
  ["warmup", "coordination", "main", "finisher", "skateskill"].forEach(bk => {
...
export function buildJourney() {
  ...
  // Chapter label comes straight from the rank's lore card (single source).
...
export function buildTodayVM(state) {
  ...
  const _BLOCK_DEFS = [
    ...
    { key: "skateskill", icon: "⛸️", label: "Skate-Skill" }
  ];
  ...
  const weather = state.weather || { icon: "☀️", temp: "–", caption: "Rink day!" };
