/* ============================================================
   ...
   data: skate_sessions_v2 history, skate_journey_v1 XP/prizes, and
...
export function buildProgressVM(state) {
  ...
  // Ice Story — future ranks stay locked as mystery cards.
  const iceStory = LADDER.map(r => {
    ...
    return {
      ...
      skate: isLocked ? "" : (lore.skate || ""), fact: isLocked ? "" : (lore.fact || ""),
  ...
  if (!milestones.length) milestones.push({ icon: "🌱", label: "Your first glide is one GO away!", style: chip("var(--aqua-wash)", "var(--aqua-ink)") });
  ...
  return {
    level, iceStory, analyticsWeek, milestones,
