...
export function buildSessionVM(state) {
  ...
  const isPrompt = phase === "intent" || phase === "microloop" || phase === "breath" || phase === "landing";
  const isBigRest = phase === "roundRest" || phase === "sectionRest";
  const timerIsReps = phase === "reps";
  const timerIsTime = !timerIsReps && !isPrompt;

  const bzMap = { warmup: "warmup", coordination: "work", main: "work", prep: "work", finisher: "rest", skateskill: "rest", recovery: "rest" };
  ...
  const bvMap = { warmup: "sun", coordination: "sun", main: "aqua", prep: "grape", finisher: "mint", skateskill: "sea", recovery: "grape" };
  ...
  const blockLabel = ({ warmup: "Warm-Up 🔥", coordination: "Coordination ⚡", main: "Main Circuit 💪",
    prep: "Prep Pair 🎯", finisher: "Finisher 🏁", skateskill: "Skate-Skill ⛸️", recovery: "Recovery ❄️" })[circuit.block] || circuit.name || "";
  ...
  const secNames = { warmup: "Warm-Up", coordination: "Coordination", main: "Main", prep: "Prep", finisher: "Finisher", skateskill: "Skate-Skill", recovery: "Recovery" };
  ...
  const BLOCK_COLORS = { warmup: "var(--coral)", coordination: "var(--sun-ink)", main: "var(--sea)", prep: "var(--grape)", finisher: "var(--mint-ink)", skateskill: "var(--aqua-ink)", recovery: "var(--grape)" };
  const sessionExList = [];
  ...
  return {
    ...
    // prompts
    intentWords: INTENT_WORDS, microQ: MICRO_LOOP.q, microOpts: ["knee over toe", "in the arms", "in the hips"],
