/* ============================================================
   BACKUP — download / restore this skater's data as JSON.
...
export function downloadBackup() {
  ...
  const name = String(payload.profile.name || "skater").toLowerCase().replace(/[^a-z0-9]+/g, "-");
  const url = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" }));
  const a = document.createElement("a");
  a.href = url;
  a.download = `skate-backup-${name}-${todayISODate()}.json`;
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 4000);
  return payload;
}

/* Reads a File chosen in the Grown-up Zone and merges it in. Resolves with a
   message for the UI; rejects with a readable one. */
export function restoreBackupFile(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      const who = settings.athleteName || "this skater";
      const bits = [];
      ...
      resolve({
        ...res,
        message: bits.length
          ? `Restored: ${bits.join(", ")}. Nothing already here was changed.`
